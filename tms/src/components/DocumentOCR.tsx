...
export default function DocumentOCR() {
  ...
  const handleCreateLoad = async () => {
    try {
      const payload = {
        load_id: '',
        origin_dest: `${ocrData.origin || ''} → ${ocrData.destination || ''}`,
        pickup_date: ocrData.pickup_date || '',
        driver: ocrData.driver || 'Unassigned',
        rate: ocrData.rate || '',
        status: 'Pending'
      };
      await fetch('http://127.0.0.1:8000/api/loads', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
      alert('Load Created Successfully! It is now visible on the Dispatch board.');
      setUploadState('idle');
      setOcrData(null);
    } catch (e) {
          ...
          {uploadState === 'done' && (
            <>
              ...
              <button 
                ...
                style={{ width: 'auto', padding: '8px 16px', borderRadius: '8px', background: 'var(--bg-surface-hover)', color: 'var(--bg-base)', border: 'none', marginTop: '16px' }}
              >
                Upload another
              </button>
            </>
        ...
        <div className="card animate-fade-in" style={{ width: '400px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1 }}>
            <div>
              ...
              <input type="text" value={ocrData?.broker || ''} onChange={(e) => setOcrData({...ocrData, broker: e.target.value})} className="search-bar" style={{ width: '100%', marginTop: '4px', color: 'var(--text-primary)' }} />
            </div>
            
            <div style={{ display: 'flex', gap: '16px' }}>
              <div style={{ flex: 1 }}>
                <label style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Origin</label>
                <input type="text" value={ocrData?.origin || ''} onChange={(e) => setOcrData({...ocrData, origin: e.target.value})} className="search-bar" style={{ width: '100%', marginTop: '4px', color: 'var(--text-primary)' }} />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Destination</label>
                <input type="text" value={ocrData?.destination || ''} onChange={(e) => setOcrData({...ocrData, destination: e.target.value})} className="search-bar" style={{ width: '100%', marginTop: '4px', color: 'var(--text-primary)' }} />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '16px' }}>
              <div style={{ flex: 1 }}>
                ...
                <input type="text" value={ocrData?.pickup_date || ''} onChange={(e) => setOcrData({...ocrData, pickup_date: e.target.value})} className="search-bar" style={{ width: '100%', marginTop: '4px', color: 'var(--text-primary)' }} />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Rate ($)</label>
                <input type="text" value={ocrData?.rate || ''} onChange={(e) => setOcrData({...ocrData, rate: e.target.value})} className="search-bar" style={{ width: '100%', marginTop: '4px', color: 'var(--text-primary)' }} />
              </div>
            </div>

            <div>
              <label style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Commodity</label>
              <input type="text" value={ocrData?.commodity || ''} onChange={(e) => setOcrData({...ocrData, commodity: e.target.value})} className="search-bar" style={{ width: '100%', marginTop: '4px', color: 'var(--text-primary)' }} />
            </div>
          </div>

          <button onClick={handleCreateLoad} className="icon-button" style={{ width: '100%', borderRadius: '8px', background: 'var(--primary)', color: 'var(--bg-base)', border: 'none', marginTop: '24px', display: 'flex', gap: '8px' }}>

...
export default function ProfitPredictorPage() {
  ...
  const handlePredict = async () => {
    ...
    setPrediction(null);
    try {
      const res = await fetch('/api/dispatch/predict', {
        method: 'POST',
        ...
        <div className="card" style={{ gridColumn: 'span 4' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginTop: '16px' }}>
            <div>
              ...
              <input type="text" value={origin} onChange={(e) => setOrigin(e.target.value)} className="search-bar" style={{ width: '100%', marginTop: '4px', background: 'rgba(255,255,255,0.05)' }} />
            </div>
            <div>
              <label style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Destination</label>
              <input type="text" value={destination} onChange={(e) => setDestination(e.target.value)} className="search-bar" style={{ width: '100%', marginTop: '4px', background: 'rgba(255,255,255,0.05)' }} />
            </div>
            <div>
              <label style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Offered Rate ($)</label>
              <input type="number" value={rate} onChange={(e) => setRate(e.target.value)} className="search-bar" style={{ width: '100%', marginTop: '4px', background: 'rgba(255,255,255,0.05)' }} />
            </div>
            <button onClick={handlePredict} disabled={loading} className="icon-button" style={{ width: '100%', borderRadius: '8px', background: 'var(--primary)', color: 'white', border: 'none', marginTop: '8px', opacity: loading ? 0.7 : 1 }}>
